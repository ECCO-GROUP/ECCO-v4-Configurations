...
C as an error then it means you have not placed your customized "tamc.h"
C file in the appropriate place.
C You need to place you own copy of tamc.h in the include path for the
C model (e.g., where your SIZE.h is), and comment out these lines.
C In particular the parameters nchklev_1/2/3 (and possibly also maxpass
C  and maxcube in case you are using ptracers or cubed sphere grid)
C need to be set correctly.
...
C     nthreads_chkpt :: Number of threads to be used; nth_chkpt .eq. nTx*nTy
      ...
      integer    nchklev_2
      parameter( nchklev_2      =  130 )
c      parameter( nchklev_2      =  150 )
      integer    nchklev_3
      parameter( nchklev_3      =  130 )
